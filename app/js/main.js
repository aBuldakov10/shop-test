...
(function () {
  ...
  $.getJSON('catalog.json', function (data) {
    ...
    $bestGoodsWrapper.append(renderGoodItem); // Slick plugin for best goods
  ...
  $headerCart.on('click', function () {
    ...
    $popupCartWrap.addClass('open'); //Empty variable for render

    var cartRender = ''; // Render form array after chosen products
  ...
  $bestGoodsWrapper.on('click', 'button', function () {
    var thisParent = $(this).closest('.good-item'),
        ...
        cartProductItem = {}; // Create object with product item data

    cartProductItem.productId = thisId;
    cartProductItem.productImg = thisImg;
    cartProductItem.productTitle = thisTitle;
    cartProductItem.productPrice = thisPrice; // Add object to array

    cartArray.push(cartProductItem); // Remove duplicates function
    ...
    cartArray = cleanArray; // Count total price
    ...
    $('header .cart .price').html(totalPrice.toFixed(2) + ' грн.'); // Add amount to cart
